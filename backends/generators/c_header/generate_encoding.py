...
def load_exception_codes_from_resolved(resolved_codes_file):
    """Load exception codes from resolved JSON file generated by Ruby task."""
    try:
        ...
        for code in resolved_codes:
            num = code.get("num")
            sanitized_name = code.get("sanitized_name")
            display_name = code.get("name")

            if num is not None and sanitized_name and display_name:
                exception_codes.append((num, sanitized_name, display_name))

        logging.info(f"Loaded {len(exception_codes)} resolved exception codes from {resolved_codes_file}")
        return exception_codes

    except Exception as e:
        logging.error(f"Error loading resolved exception codes from {resolved_codes_file}: {e}")
        return []


def load_exception_codes(ext_dir, enabled_extensions=None, include_all=False):
    """Load exception codes from extension YAML files (fallback method)."""
    exception_codes = []
    found_extensions = 0
    found_files = 0
            ...
            try:
                ...
                for code in data.get("exception_codes", []):
                    ...
                    if num is not None and name is not None:
                        # Simple sanitization without ERB processing
                        sanitized_name = (
                            name.lower()
                            .replace(" ", "_")
                            .replace("/", "_")
                        ...
                        exception_codes.append((num, sanitized_name, name))
...
def main():
    ...
    parser.add_argument(
        "--resolved-codes",
        help="JSON file containing resolved exception codes (generated by Ruby task)",
    )
    parser.add_argument(
        "--output",
    ...
    if args.resolved_codes and os.path.exists(args.resolved_codes):
        logging.info(f"Loading resolved exception codes from {args.resolved_codes}")
        causes = load_exception_codes_from_resolved(args.resolved_codes)
    else:
